...
daily_kanji_verbs = pd.concat([df[verbs].head(100).reset_index(drop = True),new_kanji[verbs]])
daily_kanji_verbs = daily_kanji_verbs.loc[daily_kanji_verbs['動詞'] == 1].drop(['動詞'],axis=1)
...
with pd.ExcelWriter('C:/Users/ruben/Desktop/毎日の漢字.xlsx', engine='openpyxl')  as writer:
    ...
    daily_kanji_verbs.to_excel(writer, sheet_name = "動詞", index=False)
