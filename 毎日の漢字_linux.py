...
daily_kanji_readings = pd.concat([df[readings].head(85).reset_index(drop = True),new_kanji[readings]])
daily_kanji = pd.concat([df[aux].head(85).reset_index(drop = True),new_kanji[aux]])
